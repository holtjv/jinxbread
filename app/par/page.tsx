...
export default function ParPage() {
  ...
  async function loadPars(targetId: string, prods: any[], windows: any[]) {
    const { data: existingPars } = await supabase.from('customer_pars').select('*').eq('customer_id', targetId)
    const parMap: Record<string, Record<string, { quantity: number; sliced: boolean }>> = {}
    windows.forEach((w: any) => { parMap[w.id] = {}; prods.forEach((p: any) => { parMap[w.id][p.id] = { quantity: 0, sliced: false } }) })
    existingPars?.forEach((par: any) => { if (parMap[par.delivery_window_id]) parMap[par.delivery_window_id][par.product_id] = { quantity: par.quantity, sliced: par.sliced } })
    setPars(parMap); setSavedPars(parMap); setHasSavedOnce(!!(existingPars && existingPars.length > 0))
  }
  useEffect(() => {
    async function load() {
      ...
      const [prodsRes, windowsRes] = await Promise.all([supabase.from('products').select('*').eq('active', true).order('sort_order'), supabase.from('delivery_windows').select('*').eq('active', true).order('sort_order')])
      const sortedWindows = (windowsRes.data || []).sort((a: any, b: any) => a.sort_order - b.sort_order)
      const prods = prodsRes.data || []
      setProducts(prods); setDeliveryWindows(sortedWindows)
      if (customer.is_admin) {
        ...
        setSelectedCustomerId(targetId); setSelectedCustomerName(storedName || 'My account')
  ...
  async function handleCustomerChange(newId: string) {
    ...
    setSelectedCustomerId(newId); setSelectedCustomerName(c?.name || '')
    sessionStorage.setItem('adminSelectedCustomerId', newId); sessionStorage.setItem('adminSelectedCustomerName', c?.name || '')
  ...
  function updatePar(windowId: string, productId: string, field: string, value: any) {
    setPars(prev => ({ ...prev, [windowId]: { ...prev[windowId], [productId]: { ...prev[windowId][productId], [field]: field === 'quantity' ? Math.max(0, parseInt(value) || 0) : value } } }))
  }
  function colTotal(windowId: string) { return Object.values(pars[windowId] || {}).reduce((t, l) => t + (l.quantity || 0), 0) }
  function weeklyTotal(productId: string): number { return deliveryWindows.reduce((t, w) => t + (pars[w.id]?.[productId]?.quantity || 0), 0) }
  function hasSavedQty(productId: string): boolean { return deliveryWindows.some(w => (savedPars[w.id]?.[productId]?.quantity || 0) > 0) }
  const sortedProducts = products.slice().sort((a, b) => (hasSavedQty(a.id) ? 0 : 1) - (hasSavedQty(b.id) ? 0 : 1))
  async function handleSave() {
    if (!selectedCustomerId) return
    setError(null)
    const violations: string[] = []
    products.forEach(p => { const total = weeklyTotal(p.id); const min = p.minimum_quantity ?? 10; if (total > 0 && total < min) violations.push(`${p.name} requires a minimum of ${min}/week (you have ${total})`) })
    if (violations.length > 0) { setError(violations.join(' · ')); return }
    setSaving(true); setSaved(false)
    const rows: any[] = []
    deliveryWindows.forEach((w: any) => { products.forEach((p: any) => { const par = pars[w.id]?.[p.id]; if (par && par.quantity > 0) rows.push({ customer_id: selectedCustomerId, delivery_window_id: w.id, product_id: p.id, quantity: par.quantity, sliced: par.sliced || false }) }) })
    ...
    if (rows.length > 0) { const { error: insertError } = await supabase.from('customer_pars').insert(rows); if (insertError) { setError('Error saving: ' + insertError.message); setSaving(false); return } }
    setSaving(false); setSaved(true); setSavedPars(pars); setHasSavedOnce(true)
        ...
        <table className="data-table">
          <thead>
            <tr>
              ...
              {deliveryWindows.map(w => (<th key={w.id} className="center" style={{ minWidth: 80 }}><div className="day-header">{dayShort[w.day_of_week]}</div></th>))}
            </tr>
          </thead>
          <tbody>
            {sortedProducts.map((p, idx) => {
              const total = weeklyTotal(p.id); const min = p.minimum_quantity ?? 10; const underMin = total > 0 && total < min
              const hasSaved = hasSavedQty(p.id); const prevHasSaved = idx > 0 ? hasSavedQty(sortedProducts[idx - 1].id) : true; const isFirstUnsaved = !hasSaved && prevHasSaved
              return (
                <>
                  {isFirstUnsaved && (<tr key={`divider-${p.id}`}><td colSpan={1 + deliveryWindows.length} style={{ padding: '8px 0 4px 0', fontSize: 11, color: 'var(--gray-400)', textTransform: 'uppercase' as const, letterSpacing: '0.05em', borderTop: '2px solid var(--gray-100)' }}>Other products</td></tr>)}
            ...
            <tr className="totals-row">
              ...
              {deliveryWindows.map(w => (<td key={w.id} className="center">{colTotal(w.id) > 0 ? colTotal(w.id) : '—'}</td>))}
            </tr>
          </tbody>
        </table>
      </div>
      <div style={{ marginTop: 32, marginBottom: 60, display: 'flex', alignItems: 'center', gap: 16 }}>
        <button onClick={handleSave} disabled={saving} className="btn btn-primary">{saving ? 'Saving...' : hasSavedOnce ? 'Update standing order' : 'Save standing order'}</button>
