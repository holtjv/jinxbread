...
export default function OrderPage() {
  ...
  // Is it past Sunday noon right now?
  function isPastCutoff(): boolean {
    const now = new Date()
    return now.getDay() === 0 && now.getHours() >= 12
  }

  // Get the Tuesday that starts the orderable week.
  // If past Sunday noon, skip ahead to the FOLLOWING Tuesday.
  function getOrderableTuesday(): Date {
    const today = new Date()
    const day = today.getDay()
    let tueDiff = 2 - day
    if (tueDiff <= 0) tueDiff += 7
    // If it's Sunday after noon, add another 7 days to get next week's Tuesday
